...
| Token | Description |
| --- | --- |
| `%^SIPARAM_1%`, `%^SIPARAM_2%`, `%^SIPARAM_3%`, etc. | Used within a `Section` to access `In=` parameters passed. The numbering scheme starts from `1` and continues in the order the parameters were passed. These variables are discarded when the section is finished processing. |
...
Although the parameters themselves are passed by value using variables, all variables are in the scope of the entire script, so the original values can modified by referencing them by name. If required, you can use the `System,SetLocal` command to isolate variables inside the running section.
...
[Open-File]
System,SetLocal
Echo,"Opening file: %^SIPARAM_1%"
ShellExecute,Open,notepad.exe,%^SIPARAM_1%
Set,%^SOPARAM_1%,%ExitCode%
System,EndLocal
```